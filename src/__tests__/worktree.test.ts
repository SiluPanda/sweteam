...
describe("git worktree operations", () => {
  ...
  afterEach(() => {
    ...
    try { cleanupWorktrees(join(realpathSync(tmpdir()), "sweteam-wt-test-"), repoDir); } catch { /* ignore */ }
  ...
  describe("addWorktree", () => {
    it("should create a worktree with a new branch", () => {
      ...
      try { git(["branch", "-D", "task-branch-1"], repoDir); } catch { /* ignore */ }
    });
    ...
    it("should create multiple independent worktrees", () => {
      ...
      try { git(["branch", "-D", "branch-1"], repoDir); } catch { /* ignore */ }
      try { git(["branch", "-D", "branch-2"], repoDir); } catch { /* ignore */ }
    });
    ...
    it("should base worktree on the correct branch", () => {
      ...
      // Cleanup
      removeWorktree(wtPath, repoDir);
      try { git(["branch", "-D", "task-branch"], repoDir); } catch { /* ignore */ }
      try { git(["branch", "-D", "feature"], repoDir); } catch { /* ignore */ }
    });
  });
  ...
  describe("cleanupWorktrees", () => {
    it("should remove worktrees matching a path prefix", () => {
      ...
      try { git(["branch", "-D", "branch-b1"], repoDir); } catch { /* ignore */ }
    });
  });
