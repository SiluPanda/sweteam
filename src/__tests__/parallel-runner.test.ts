...
import { buildDag, getReadyTasks, topologicalSort, type DagNode } from "../orchestrator/dag.js";
...
describe("parallel runner — worktree integration", () => {
  ...
  afterEach(() => {
    ...
    try {
      const output = execFileSync("git", ["-C", repoDir, "worktree", "list", "--porcelain"], { encoding: "utf-8" });
      for (const line of output.split("\n")) {
        if (line.startsWith("worktree ") && !line.includes(repoDir.replace(/\/private/, ""))) {
          const wtPath = line.slice("worktree ".length);
          try { execFileSync("git", ["-C", repoDir, "worktree", "remove", "--force", wtPath]); } catch { /* ignore */ }
        }
      }
    } catch { /* ignore */ }
